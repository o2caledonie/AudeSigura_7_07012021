...
exports.signup = (req, res, next) => {
    ...
    if (!/(?=^.{8,}$)(?=.*\d)(?=.*[!@#$%^&*]+)(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/.test(req.body.password)) {
        return res.status(400).json({ error: 'Le mot de passe doit contenir minimum 8 caractères avec au minimum unle lettre minuscule et une majuscule, un chiffre et un caractère spécial !@#$%^&*' })
    };
